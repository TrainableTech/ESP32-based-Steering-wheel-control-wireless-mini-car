...
CytronMD motor1(PWM_PWM, 6, 7);   // PWM 1A PWM 1B
CytronMD motor2(PWM_PWM, 8, 9); // PWM 2A PWM 2B
...
/* Pitch and roll consider all axes for calculation. According to my experience
  ...
  if((pitch >= 10 && pitch <= 70)||(pitch >= -70 && pitch <= -10))    
 {
   if(pitch >= 10 && pitch <= 70) 
   {
   motor1.setSpeed(128);
   }
   else if(pitch >= -70 && pitch <= -10)
   {     
  ...
  if((roll >= 10 && roll <= 70)||(roll >= -70 && roll <= -10))    
 {
   if(roll>=10 && roll<=70)
   {
    motor2.setSpeed(128);
   }
   else if(roll >= -70 && roll<= -10)
   {
   motor2.setSpeed(-128);
   }
 }
  ...
  { 
    // Serial.println("DEAD Motion ROLL");
    motor1.setSpeed(0);
    motor2.setSpeed(0);
  }
